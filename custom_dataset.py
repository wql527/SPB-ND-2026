...
class YOLODataset(BaseDataset):
    ...
    def __init__(self, *args, data: dict | None = None, task: str = "detect", **kwargs):
        ...
        self.epoch = 0
        self.toatl_epochs = None

    def update_augmentations(self, hyp):
        """
        Curriculum learning: progressively increase augmentation difficulty
        """
        if self.total_epochs is None:
            return

        p = self.epoch / self.total_epochs  # [0,1]  # ★★★★customize

        # 前期简单，后期困难（你可自行调）
        hyp.mosaic = min(1.0, p * 1.2)
        hyp.copy_paste = max(0.0, p - 0.2)
        hyp.mixup = max(0.0, p - 0.4)
        hyp.cutmix = max(0.0, p - 0.6)
